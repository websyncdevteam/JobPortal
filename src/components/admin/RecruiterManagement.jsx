...
const RecruiterManagement = () => {
  ...
  // 📊 Fetch all data - FIXED: No broken API calls
  const fetchData = async () => {
    try {
      ...
      // Fetch companies - ADMIN ENDPOINT
      const companiesResponse = await api.get('/admin/companies');

      // Handle responses
      const recruitersData = recruitersResponse.data?.data?.recruiters || 
                            recruitersResponse.data?.recruiters || 
      ...
      // Generate mock activities from recruiter data (fallback)
      const mockActivities = generateMockActivities(recruitersData);
  ...
  const bulkAssignCompany = async (companyId) => {
    try {
      await api.post('/admin/recruiters/assign-multiple-companies', {
      ...
      setError({ type: 'success', message: `Assigned company to ${selectedRecruiters.length} recruiters` });
    } catch (err) {
      setError({ type: 'error', message: 'Failed to assign company' });
    }
  };
    ...
    try {
      await Promise.all(
        selectedRecruiters.map(id => 
          api.put(`/admin/user/suspend/${id}`, { suspend: true })
        )
      );
      setSelectedRecruiters([]);
      fetchData();
      setError({ type: 'success', message: `Suspended ${selectedRecruiters.length} recruiters` });
    } catch (err) {
  ...
  const bulkVerify = async () => {
    try {
      await Promise.all(
        selectedRecruiters.map(id => 
          api.put(`/admin/recruiters/${id}/verify`, { verified: true })
        )
      );
      setSelectedRecruiters([]);
      fetchData();
      setError({ type: 'success', message: `Verified ${selectedRecruiters.length} recruiters` });
    } catch (err) {
                ...
                <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                  <div className="flex items-center justify-between">
                    ...
                    <QuickActionsMenu 
                      ...
                      onVerify={() => {/* Verification logic */}}
                      onSuspend={() => {/* Suspend logic */}}
