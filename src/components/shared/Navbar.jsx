...
import api from "@/services/api"; // ✅ Use central API instance
...
const Navbar = () => {
  ...
  const logoutHandler = async () => {
    try {
      const res = await api.get("/user/logout"); // ✅ Central API instance
      if (res.data.success) {
  ...
  const getUserFirstName = () => (user?.fullname ? user.fullname.split(" ")[0] : "User");
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link
            to="/"
            className="flex items-center focus:outline-none focus:ring-2 focus:ring-purple-500 rounded-lg transition-transform hover:scale-105"
          >
            ...
            {!user ? (
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="flex items-center space-x-3 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded-lg p-2 hover:bg-purple-50 transition-colors"
                >
                  ...
                  <Avatar className="w-10 h-10 border-2 border-purple-100">
                    <AvatarImage
                      src={getProfilePicture() || `https://ui-avatars.com/api/?name=${getUserInitials()}&background=7c3aed&color=fff`}
                      alt={user?.fullname}
                    />
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50 animate-in fade-in slide-in-from-top-5">
                    ...
                    {user?.role === "Freelancer" && (
                      <>
                        <button onClick={goToDashboard} className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                          <LayoutDashboard size={18} /> Dashboard
                        </button>
                        <button onClick={goToProfile} className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                          <User2 size={18} /> Profile
                        </button>
                      </>
                    )}
                    {user?.role === "candidate" && (
                      <button onClick={goToCandidateProfile} className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                        <User2 size={18} /> View Profile
                      </button>
                    )}

                    <div className="border-t border-gray-100 my-1"></div>
                    <button onClick={logoutHandler} className="flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 transition-colors">
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden transition-all duration-300 ease-in-out overflow-hidden ${mobileMenuOpen ? "max-h-screen opacity-100 bg-white border-t border-gray-100 shadow-lg" : "max-h-0 opacity-0"}`}>
        <div className="px-4 py-6 space-y-4">
          {/* ... Repeat links & role-based buttons for mobile ... */}
          {/* For brevity, you can copy the desktop links here */}
        </div>
      </div>
    </nav>
  );
